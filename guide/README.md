...
### Repo packaging (multi-page requirement)

Implementation docs should be structured as a multi-page repo, not a single long `README.md`.

- Keep the client `README.md` as the entry point (introduction + scope + links).
- Put detailed workflows, mappings, and edge cases into dedicated pages under `flows/` (one folder per flow with its own `README.md`).
- Always include a `SUMMARY.md` so navigation is complete.
