...
HotWax Commerce’s Inventory Receiving App enables stock associates to efficiently receive ASN, Purchase Orders, and Returns and update the inventory count in HotWax Commerce Order Management System.

The Inventory Receiving App enables retailers to precisely track the physical inventory received at their locations and record them in the system. For example, if a store anticipates 100 shirts in an incoming shipment but physically receives only 90, the app facilitates in recording exact inventory count. This distinction is crucial as it prevents potential order rejections during fulfillment due to discrepancies in inventory levels.
...
The first page users see when they log into the app is the `Shipments` page which houses a complete list of ASN receivied from ERP or WMS. To locate any specific ASN, store associates can scan the ASN barcode from the physical copy received along with the package. Store associates can also manually input the ASN ID in the `Search` box to locate the shipment.
...
The inbound shipments generally contain multiple product items, store associates can either scan individual product barcodes or search SKUs to locate specific items and start receiving. In case you are not managing barcodes, tap on the `image icon` to see the enlarged image of the item to easily identify the product before receiving.
...
Retailers who accept return at their stores need to accurately receive and record the returned inventory for potential resale on e-commerce. Store associates can find the `Returns` option on the top left corner in the app below `Shipments`.
...
Purchase orders are buyer-generated documents detailing product specifics, quantities, and prices. Retailers use these purchase orders to order from suppliers. Upon delivery, retailers need to verify the received inventory against the purchase order, ensuring accurate shipments and inventory counts. Store associates can find the `Purchase Order` option on the top left corner in the app below `Returns`.
...
At times, stores receive items that differ from the recorded shipment. This discrepancy could arise from mispicking at the warehouse or inadvertently shipping more items than intended. When stores unexpectedly receive these extra items, they require a process to receive and record these additional items effectively. For such cases, a `plus button (+)` is provided on the top right corner of the app to add the missing items from the shipments to the inventory. Furthermore, the platform generates reports for these items, allowing retailers to receive this extra inventory efficiently within their ERP system.
