---
description: >-
  Learn more about ADOC's order invoicing process, ensuring accurate invoicing
  and inventory adjustments for fulfilled orders.
---
...
HotWax uses the OrderToInvoice API from RetailPro to sync all completed orders for invoicing. In cases of split orders, where an item is fulfilled from one store but invoiced at another, Retail Pro’s Inventory Transfer API is utilized to adjust inventory to effectively make it look like the entire order was shipped from one store.
