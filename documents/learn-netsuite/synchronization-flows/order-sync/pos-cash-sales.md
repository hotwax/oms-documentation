...
Point of Sale (POS) orders are already in “Completed” status in the Order Management System (OMS). A feed file for these orders is prepared and kept on the SFTP server at `/home/{sftp-username}/netsuite/cashsale/export`. The `HC_SC_ImportCashSale` SuiteScript downloads the CSV file from the SFTP and creates sales records in NetSuite. To check whether the CSV file is successfully imported, go to `Customization > Import/Export > View CSV Records`. The `HC_MR_ExportedCashSaleCSV` script exports the CSV file with the NetSuite Internal ID and puts it on the SFTP. The `Order Identification` job in the OMS then consumes the CSV file from the SFTP to create Order Identification records.
