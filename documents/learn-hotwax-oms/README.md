...
In the context of HotWax Commerce, store associates receive ASNs in relation to Transfer Orders (TOs). Generally, a Transfer Order is created in an ERP system, and when the TO is fulfilled in the ERP or Warehouse Management System (WMS), an ASN is generated in HC. Store associates then perform the receiving based on this ASN using the receiving app. In the context of Purchase orders, the ASN receiving is done in ERP/WMS.
