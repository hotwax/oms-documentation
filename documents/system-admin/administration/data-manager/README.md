...
### Configurations
A data manager configuration represents the import settings for a type of data. For example, importing a sales orders from Shopify, fulfillment from a 3PL, or inventory from a POS are all separate configurations because they are all importing something different.
...
Essentially what happens when you assign an import service to an MDM, is the OMS uses that service to loop through the file submitted to it row by row. When setting up an integration, enriching data in the OMS, or fixing corrupted data, this allows you to quickly turn any simple function into a bulk function with multi-threading and transaction management.

To make the MDM more approachable for starters, we've identified the most commonly used configurations and organized them on the EXIM (Export Import) page. You can see all configurations though in the Data Manager Configurations page.
...
All orders placed after the morning inventory file is submitted will not be processd in the OMS until the reset inventory file is finished processing. While this may seem problematic at first because orders are not being processed as they're being placed, the time of day when this operation happens is important to cnsider. Inventory update processing is happening somewhere between 12am - 4am during which no fulfillment operations are running so orders processing after the inventory file finishes does not actually hurt a retailers fulfillment SLA.

#### Queue poller
All file import tasks enter the MDM queue in a `Pending` status. As the poller works its way through the queue, it transitions the current active file to a `Running` status. The frequence that the poller checks for pending files can be configured using the Process Bulk Imported Files job in the job manager app. Every time the poller runs, it registers all `Pending` items in the queue and will continue to run until all the pending files at the time of polling are finished processing.

During this time other scheduled occurances of the queue poller job, Process Bulk Imported Files, may take place, but seeing that there is already an instance of the file processer active, it will cancel itself. In the job manager app this may look like an error that needs to be resolved but is normal behavior.