...
- **Transfer Order Name**: Displays the name of the transfer order. Just above it, the system shows a unique ID auto-generated by the app.  
- **Transfer Order Status**: The status appears next to the order name, as Approved or Completed.  
  - If any items are pending fulfillment, the order remains in Approved status.  
  - Once all items are fulfilled, the status changes to Completed.  
