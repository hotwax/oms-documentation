...
<figure><img src="../../.gitbook/assets/chooseOptimallocations.png" alt=""><figcaption><p>Choose Optimal Facilities</p></figcaption></figure>
...
* **Shipment Threshold Value Check:** Retailers can configure a minimum shipment value to prevent order splitting when the item value is too low. This helps avoid the cost of shipping low-value items separately, while still maintaining a balance between delivery speed and overall shipping efficiency. For example, if the threshold is set at $25, only items or item groups exceeding this value will be split and shipped from different locations.
