...
* **Week of Supply:** Retailers can add a Weeks of Supply value to calculate inventory coverage using current inventory and sales velocity. This value is used by the `Week of Supply` sort option to prioritize facilities with the deepest inventory cover. For example, a slow-moving outlet store with more weeks of cover can be attempted before a high-velocity store that needs its inventory for walk-in customers. Learn more about [Weeks of Supply Routing](weeks-of-supply-routing.md).
...
* **Week of Supply:** Facilities can be sorted by the amount of inventory cover they have for the ordered item. When a Weeks of Supply value is configured and the `Week of Supply` sort is selected, HotWax Commerce attempts facilities with higher coverage first. This helps route online orders to locations that can spare inventory and away from stores where local demand is already consuming stock quickly.
