...
6. For sections with a `Save` footer, save each changed card. Select `Reset` to discard that card's unsaved draft. If the app reports `Underlying data changed`, review the latest data before saving again.
...
5. Update the matching area in `Product details`:
   ...
   * Use `Categories` to add the intended category when `Primary category` is missing. The app doesn't provide a control to designate a category as primary, so escalate the product ID if the gap remains.
...
**Use this flow when:** A shop-originated product change isn't visible in Product details or the Product workbench. Use it also to compare recent snapshots for a product or shop.
